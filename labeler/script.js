...
document.addEventListener("DOMContentLoaded", function () {
  ...
  let initialX, initially;
  const createdTextElements = [];

  function createDraggable(text) {
    const draggable = svgContainer
      .append("g")
      .attr("class", "draggable")
      .attr("transform", "translate(50, 50)");

    const textElement = draggable
      .append("text")
      .attr("x", 0)
      .attr("y", 0)
    ...
    draggable.on("mousedown", function (e) {
      ...
      const transformedPoint = point.matrixTransform(
        svgContainer.node().getScreenCTM().inverse(),
      );
      offsetX =
        transformedPoint.x -
        parseFloat(this.getAttribute("transform").split("(")[1]);
      offsetY =
        transformedPoint.y -
        parseFloat(this.getAttribute("transform").split(",")[1]);
      initialX = e.clientX;
    ...
    d3.select(document).on("mousemove", function (e) {
      ...
      const deltaY = e.clientY - initially;
    ...
    textElement.on("dblclick", function () {
      const index = createdTextElements.findIndex(
        (elem) => elem.textElement.node() === this,
      );
      if (index !== -1) {
    ...
    textInput.on("input", function () {
      ...
      const index = createdTextElements.findIndex(
        (elem) => elem.textElement.node() === textElement.node(),
      );
      if (index !== -1) {
    ...
    xInput.on("input", function () {
      ...
      if (index !== -1) {
        ...
        createdTextElements[index].textElement.attr(
          "transform",
          `translate(${newX}, ${0})`,
        );
  ...
  function loadSvg() {
    ...
    if (file) {
      ...
      reader.onload = function (event) {
        ...
        svgContainer.attr("width", svgWidth).attr("height", svgHeight); // Ajustar dimensions del contenedor
      };
  ...
  function addRow() {
    ...
    for (let i = 0; i < values.length; i++) {
      ...
      cell.classList.add(
        `${["name", "x", "y", "size", "rotation", "color", "font"][i]}-input`,
      );
    }
  ...
  document
    ...
    .addEventListener("click", function () {
      ...
      const fileNameWithoutExtension = prompt(
        "Ingresa el nombre deseado para el archivo (sin la extensión):",
        "tabla",
      );
  ...
  function downloadSVG() {
    ...
    const fileNameWithoutExtension = prompt(
      "Ingresa el nombre deseado para el archivo SVG:",
      "modified",
    );
  ...
  function downloadPDF() {
    ...
    doc.on("end", () => {
      ...
      const fileNameWithoutExtension = prompt(
        "Ingresa el nombre deseado para el archivo PDF:",
        "downloaded",
      );
      if (!fileNameWithoutExtension) return;
