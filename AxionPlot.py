...
from DataBaseGag import *

class AxionGagPlottest:
    # ==============================================================================#
    # build and plot...
    #
    ...
    def __init__(self, plottype="large_panorama", projections=False, showplot=True, saveplot=True):
        ...
        #Create Data Base
        self.axionDB = BuildGagDB()
        #print(self.axionDB)

        #Plotting Data & Labels
        self.plotdatatest(plottype, projections)
        #self.PlotLabelstest(plottype, projections)
    ...
    def plotdatatest(self, plottype, projections = False):
        ...
        if plottype in ["haloscopes", "haloscopes_zoom", "haloscopes_radeszoom"]:
            plottype = "haloscopes"
            data = Search(plottype ,1)
            names = Search_Names(plottype ,1) 
            for row, name in zip(data, names):
                last_token = row[-1]  # Access the last token of each row in the data
                RenormItem(self.axionDB[name])#Its done in Haloscopes exclusively
                if projections:
                    self.axionDB[name].DrawItem(self.axplot)
                elif not projections and last_token == 0:
                    self.axionDB[name].DrawItem(self.axplot)
        else:
            data = Search(plottype ,1)
            names = Search_Names(plottype ,1) 
            for row, name in zip(data, names):
                last_token = row[-1]  # Access the last token of each row in the data (last_token= proyection token if 1 is proyection)
                if projections:
                    self.axionDB[name].DrawItem(self.axplot)
                elif not projections and last_token == 0:
                    self.axionDB[name].DrawItem(self.axplot)
    ...
    def PlotLabelstest(self, plottype, projections=False):
        # ===========================================================================#
        if plottype == "large_panorama":
            plt.text(1e-10, 2e-10, r'{\bf Helioscopes (CAST)}', color="black", size=10)
            plt.text(1e-9, 2e-7, r'{\bf Laboratory}', color="white", size=10)
...
def BuildGagDB():
    ...
    db['ADMX_SLIC'] = ExPltItem("ADMX_SLIC", "band", path + "ADMX_SLIC.txt", facecolor="limegreen",
                                edgecolor="darkgreen", linewidth=0.5, alpha=1)

    db['SHAFT'] = ExPltItem("SHAFT", "band", path + "SHAFT.txt", facecolor="limegreen", edgecolor="darkgreen",
                            linewidth=0.5, alpha=0.5)
    #print('voy por la mitad')DM	band	/home/tfg_2022_1/git/axion-limits/data/axion/data/axion/IAXODM.dat	0	0
    ...
    db['HBalpbound'] = ExPltItem('HBalpbound', 'band', path + 'HBalpbound.txt', facecolor="skyblue", edgecolor="blue",alpha=1,
                                 linewidth=0.5)
    ...
    db['KLASH_l'] = ExPltItem('KLASH', 'line', path + 'KLASH.dat', color="darkgreen", linewidth=0.1, alpha=1,
                              linestyle="--")
    db['IAXODM_l'] = ExPltItem('IAXODM', 'line', path + 'IAXODM.dat', color="darkgreen", linewidth=0.1, alpha=1,
                               linestyle="--")
